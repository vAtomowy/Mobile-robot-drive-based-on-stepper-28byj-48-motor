...
void InitStepper(DRIVE*drive_struct, int Pin_1, int Pin_2, int Pin_3, int Pin_4, int vel ){ 
    ...
    drive_struct->threshold_pose = 3*(1.0f/2048.0f);
    drive_struct->one_fullstep = (1.0f/2048.0f);
    drive_struct->one_halfstep = (1.0f/4096.0f);
...
void DrivesTask(DRIVE*drive_struct){
    ...
    {   // i jeśli pozycja poza zakresem zadanym 
        //if(!(((drive_struct->pose)-(drive_struct->threshold_pose)) >= (drive_struct->ref_pose) ))
